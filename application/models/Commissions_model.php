...
class Commissions_model extends App_Model
{
    ...
    {
        $this->db->select('slug,articleid, articlegroup, subject, amount,' . db_prefix() . 'commissions.description,' . db_prefix() . 'commissions.active as active_article,' . db_prefix() . 'commissions_groups.active as active_group,name as group_name,staff_article');
        $this->db->from(db_prefix() . 'commissions');
        $this->db->join(db_prefix() . 'commissions_groups', db_prefix() . 'commissions_groups.groupid = ' . db_prefix() . 'commissions.articlegroup', 'left');
    ...
    public function get_related_articles($current_id, $customers = true)
    {
        ...
        $article = $this->db->get(db_prefix() . 'commissions')->row();
        ...
        return $this->db->get(db_prefix() . 'commissions')->result_array();
    }
    ...
    public function add_article($data)
    {
        ...
        $slug_total = $this->db->count_all_results(db_prefix() . 'commissions');
        if ($slug_total > 0) {
        ...
        $this->db->insert(db_prefix() . 'commissions', $data);
    ...
    public function update_article($data, $id)
    {
        ...
        $this->db->where('articleid', $id);
        $this->db->update(db_prefix() . 'commissions', $data);
    ...
    public function update_kan_ban($data)
    {
        ...
        foreach ($data['order'] as $o) {
            ...
            $this->db->update(db_prefix() . 'commissions', [
                'article_order' => $o[1],
    ...
    public function update_groups_order()
    {
        ...
        foreach ($data['order'] as $group) {
            ...
            $this->db->update(db_prefix() . 'commissions_groups', [
                'group_order' => $group[1],
            ]);
        }
    }
    ...
    {
        $this->db->where('articleid', $id);
        $this->db->delete(db_prefix() . 'commissions');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('articleid', $id);
    ...
    public function get_kbg($id = '', $active = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix() . 'commissions_groups')->row();
        }
        $this->db->order_by('group_order', 'asc');

        return $this->db->get(db_prefix() . 'commissions_groups')->result_array();
    }
    ...
    public function add_group($data)
    {
        ...
        $slug_total = $this->db->count_all_results(db_prefix() . 'commissions_groups');
        if ($slug_total > 0) {
            $data['group_slug'] .= '-' . ($slug_total + 1);
        }

        $this->db->insert(db_prefix() . 'commissions_groups', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_group($data, $id)
    {
        ...
        $this->db->where('groupid', $id);
        $this->db->update(db_prefix() . 'commissions_groups', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_group($id)
    {
        ...
        if (is_reference_in_table('articlegroup', db_prefix() . 'commissions', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('groupid', $id);
        $this->db->delete(db_prefix() . 'commissions_groups');
        if ($this->db->affected_rows() > 0) {
